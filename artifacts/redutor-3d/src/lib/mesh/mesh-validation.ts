...
export function validateMeshGeometry(mesh: any, originalMesh: any, config: any): any {
  ...
  const hausdorffDistance = computeHausdorffDistanceSimple(
    mesh.positions, originalMesh.positions
  );
  ...
  const selfIntersections = detectSelfIntersections(mesh.positions, mesh.indices);
  ...
  // Compute Hausdorff distance (simplified)
  const hausdorffDistance = computeHausdorffDistanceSimple(mesh.positions, originalMesh.positions);
...
function computeHausdorffDistanceSimple(positions1: Float32Array, positions2: Float32Array): number {
  let maxDist = 0;
